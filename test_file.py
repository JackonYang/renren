...
class test_repo_file(unittest.TestCase):
	...
	def tearDown(self):
		...
		self.repo=None  # save when __del__
		clear(f)

	def test_save_friendList(self):
		names=[
			({'266754031':'王瑛','27331442':'Ethan.王哲','240303471':'刘洋English','239439171':'','222439171':'eeee','324134134':'～！@#￥%……&*（）'},6),
			(dict(),0),
			(None,None),
			({'3','4'},None)
			]
	...
	def test_getSearched(self):
		...
		self.repo.__del__()
		self.repo2=new_file()
		self.repo2.save_friendList({'92':'name2'},'12','timecost_info')
		self.repo2.save_status(stat1,'21','success')
	...
	def test_getFriendList(self):
		names=[
			...
			({'231':'王瑛','2442':'Ethan.王哲','241':'刘洋English','2171':'','439171':'eeee','324134134':'～！@#￥%……&*（）'},'2000123')
		]
		for name,rid in names:
			self.repo.save_friendList(name,rid)
			self.assertEquals(self.repo.getFriendList(rid),set(name.keys()))
		self.repo.__del__()
